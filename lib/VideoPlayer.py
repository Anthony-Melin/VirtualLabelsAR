...
class VideoPlayer(Video):
    ...
    def read(self):
        ...
        elif self.status == VideoPlayer.PLAY and self.getRecordTime() >= self.TIME:
            self.stop()
            return False, None
    ...
    def test(config):
        ...
        # popup for video filename
        root = tk.Tk()
        video_filename = askopenfilename()
        root.destroy()

        # if filename is valid
        if video_filename and video_filename.endswith((".mp4")):
            
            # instanciates new Video object
            videoPlayer = VideoPlayer(video_filename)
            ...
            scale = tk.Scale(root, orient=tk.HORIZONTAL, variable=scale_var, command=videoPlayer.setCurrentTime, to=19)
